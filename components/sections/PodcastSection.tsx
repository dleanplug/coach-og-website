...
import { podcastTopics } from "@/lib/constants";
...
export function PodcastSection() {
  return (
    <section
      id='podcast'
      className='reveal page-padding py-[100px] bg-secondary text-white  max-lg:py-[72px]'>
      <div className='grid grid-cols-2 gap-16 items-center max-lg:grid-cols-1 max-lg:gap-12'>
        ...
        <div>
          <SectionHeading
            ...
            title={
              <>
                ...
                <em className='italic text-accent'>Move You</em>
              </>
            }
            light
            className='mb-3'
          ...
          <p className='text-[0.9rem] leading-[1.8] text-white/60 mb-6 max-w-[480px]'>
            Deep dives into behavioral psychology, market dynamics, career
            positioning, and the unspoken truths of sustainable success. Listen
            weekly across all major platforms.
          </p>
          ...
          <div className='flex flex-wrap gap-2 mb-8'>
            {podcastTopics.map((topic, index) => (
              <div
                key={index + "we"}
                className='py-1 px-3.5 text-[0.65rem] text-white/45 tracking-[1.5px] uppercase border border-accent/30 rounded-full'>
                {topic}
              </div>
            ))}
          </div>
          ...
          <div className='flex gap-4 mb-2 flex-wrap'>
            {["Spotify", "Apple Podcasts", "Google Podcasts"].map(
              (platform, index) => (
                <a
                  key={platform}
                  href='#'
                  className={cn(
                    "bg-white/5 border border-white/10 px-5 py-2.5 rounded-[2px] text-[0.7rem] tracking-[1.5px] uppercase text-white/80 transition-all hover:bg-accent hover:text-secondary hover:border-accent",
                    index === 0 && "bg-primary",
                  )}>
                  {platform}
                </a>
        ...
        <div className='relative p-10 bg-linear-to-br from-[#16234b] to-secondary border border-white/8 rounded-[4px] shadow-[0_30px_60px_rgba(0,0,0,0.4)]'>
          ...
          <p className='text-[0.8rem] text-white/50 leading-[1.6] mb-6'>
            In this episode, Coach OG breaks down the invisible barriers that
            prevent SMEs from scaling beyond their first major revenue
            milestone...
          </p>
